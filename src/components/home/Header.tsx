...
const Header = () => {
  ...
  const navLinks = [
    { href: '/NexusPro#features', label: 'Funcionalidades' },
    { href: '/NexusPro#testimonials', label: 'Depoimentos' },
    { href: '/NexusPro#pricing', label: 'Preços' },
    { href: '/NexusPro#faq', label: 'FAQ' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center">
            <Link href="/NexusPro" className="flex-shrink-0">
          ...
          <nav className="hidden md:flex items-center space-x-6">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} className="text-gray-600 hover:text-blue-600 dark:text-gray-300 dark:hover:text-blue-400 transition-colors font-medium">
                {link.label}
              </Link>
            ))}
          </nav>
