        ...
        try {
            ...
            if (result.success) {
                toast({
                    ...
                    description: "Sua conta foi criada. Agora, escolha o plano e configure o pagamento.",
                });
                // Redirect to a page where they select the plan and pay
                // For now, let's redirect to login, as the next step is not defined
                setTimeout(() => {
                    router.push('/login'); 
                }, 2000); 
            } else {
               ...
               <Form {...form}>
                <form onSubmit={form.handleSubmit(onFormSubmit)} className="space-y-8">
                    ...
                    <div>
                        ...
                        <div className="space-y-6">
                            ...
                            <FormField control={form.control} name="mainActivity" render={({ field }) => (
                                <FormItem><FormLabel>Atividade Principal</FormLabel><FormControl><Input placeholder="Ex: Comércio varejista de roupas" {...field} /></FormControl><FormMessage /></FormItem>
                            )} />
                            ...
                            <FormField control={form.control} name="regimeTributario" render={({ field }) => (
                                ...
                                <SelectTrigger><SelectValue placeholder="Selecione o regime da sua empresa" /></SelectTrigger></FormControl>
                                <SelectContent>{regimesTributarios.map(regime => (<SelectItem key={regime.id} value={String(regime.id)}>{regime.nome}</SelectItem>))}</SelectContent>
                                </Select><FormMessage /></FormItem>
                            )} />
                            <FormField control={form.control} name="segmentoMercado" render={({ field }) => (
                                <FormItem><FormLabel>Segmento de Mercado</FormLabel><FormControl><Input placeholder="Ex: Varejo de Roupas, Mercearia, etc." {...field} /></FormControl><FormMessage /></FormItem>
                            )} />
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <FormField control={form.control} name="diaVencimento" render={({ field }) => (
                                <FormItem><FormLabel>Dia de Vencimento da Mensalidade</FormLabel>
                                    <Select onValueChange={field.onChange} defaultValue={field.value}><FormControl>
                                    <SelectTrigger><SelectValue placeholder="Selecione um dia" /></SelectTrigger></FormControl>
                                    <SelectContent>{diasVencimento.map(dia => (<SelectItem key={dia} value={String(dia)}>Dia {dia}</SelectItem>))}</SelectContent>
                                    </Select><FormMessage /></FormItem>
                                )} />
                                <FormField control={form.control} name="emailComercial" render={({ field }) => (
                                    <FormItem><FormLabel>E-mail Comercial (para faturamento)</FormLabel><FormControl><Input type="email" placeholder="financeiro@suaempresa.com" {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                            </div>
                            <FormField control={form.control} name="instagram" render={({ field }) => (
                                <FormItem><FormLabel>Instagram (Opcional)</FormLabel><FormControl><Input placeholder="@seuinstagram" {...field} /></FormControl><FormMessage /></FormItem>
                            )} />
                        </div>
                    </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <FormField control={form.control} name="email" render={({ field }) => (
                                    <FormItem><FormLabel>E-mail (pessoal para notificações)</FormLabel><FormControl><Input type="email" placeholder="seu-email@exemplo.com" {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                            </div>
                    ...
                    <Button type="submit" className="w-full text-lg py-6" disabled={loading}>
                        ...
                        {loading ? 'Finalizando Cadastro...' : 'Criar Conta e Ir para o Plano'}
                      </Button>
                </form>
               </Form>
